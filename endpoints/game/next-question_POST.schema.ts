...
export const postGameNextQuestion = async (body: InputType, init?: RequestInit): Promise<OutputType> => {
  const validatedInput = schema.parse(body);
  const result = await fetch(`/_api/game/next-question`, {
    method: "POST",
  ...
  if (!result.ok) {
    ...
    throw new Error(errorObject.error || "Failed to advance to next question");
  }
